#Maximum entropy classifier. Type "python3 Maxent.py" to run. Currently hard coded to only test "sSept_immi.csv" with "sNov_immi.csv". Results are outputted in "maxent_immi_results.csv"
import pandas as pd
import re
import nltk
import importlib
...
NBayesClassifier=nltk.MaxentClassifier.train(trainingFeatures, max_iter=10)
...
fourth = nltk.classify.accuracy(NBayesClassifier, testFeatures)
#cnt = cnt + 1
#fourth = (fourth/cnt)
# ------------------------------------------------------------------------
...
lop.loc[0] = [pos] + [neg] + [neu] + [fourth] + [len(testDataSet.index)] + [testDataSet.loc[0, "month"]] 
#NBResultLabels.show_most_informative_features()


lop.to_csv("maxent_immi_results.csv", index=False, encoding='utf-8-sig')
